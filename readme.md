...
## Building & Deploying This Repo

This repo is a **build artifact**, not a source repo. `index.html` and nearly
everything at the root is generated by the pipeline in `app/scripts/` and
overwritten on every build — edit the game source in the sibling
`ShellShockers/game/` repo instead.

```bash
bash app/scripts/build.sh          # sync, transform, commit (never pushes)
git push origin main master        # both branches — jsDelivr serves @master too
python3 app/scripts/purge.py       # only AFTER the push
```

`build.sh` needs an interactive terminal: it prompts for a sudo password and for
"Do items need to be vaulted?". It also has a silent-stale-build failure mode.

**Read [`app/scripts/README.md`](app/scripts/README.md) before running a build** —
prerequisites, what survives the sync, and how to verify a build actually landed.

Files that survive a build: `.git`, `.gitignore`, `app/`, `readme.md`,
`functions/`, `_headers`. Everything else at the root is replaced.
